# YouYou Agent - Final Architecture Design
...
| Supersedes  | `0006_youyou_agent_design_claude.md` |
| Created     | 2026-03-18          |
| Related     | `0005_youyou_demand.md`, `0007_youyou_agent_implement.md` |
| Module Path | `src/` |
...
#### A6 — v1 不引入 Codex-style agent policy

**对齐原则：** `0005` 已明确当前 Agent 核心不访问文件系统，所有输入由调用方提供；同时，项目级/产品级约束已经可通过 `system_instructions`、`system_prompt_override`、`personality`、`environment_context` 注入模型可见上下文。

**内部实现细化：** v1 不引入 `approval_policy / sandbox_policy / exec_policy / AGENTS.md` 自动发现与注入机制，也不设计一个语义过宽的 `AgentPolicy` 抽象。若未来引入内建 shell/file/network tool 与人工审批流，再单独设计 `ExecutionPolicy` 或 `ToolAccessPolicy`。
...
│  ChatRequestBuilder, ToolDispatcher, SkillManager,   │
│  MemoryManager, PluginManager, HookRegistry          │
├──────────────────────────────────────────────────────┤
│  Domain Layer                                        │
...
```text
src/
├── lib.rs                      // crate root，re-export 公共 API
│
...
│   ├── prompt_builder.rs       // PromptBuilder：System Prompt 文本渲染
│   ├── request_builder.rs      // ChatRequestBuilder：请求组装与 capability 预检
...
│   ├── session_service.rs      // SessionService：会话生命周期应用服务（实现辅助层）
...
当前设计文档直接对应仓库内的 `src/` 目录，crate root 使用 `src/lib.rs`。若后续需要拆分成独立 crate，保持目录分层和 public API 不变，由宿主层做包装即可。
...
- v1 不额外引入 `AgentPolicy` 配置对象。
- 项目级行为约束统一通过 `system_instructions` 提供。
- 会话级临时约束通过 `system_prompt_override` 提供。
- 表达风格通过 `personality` 提供。
- 运行环境事实通过 `environment_context` 提供。
...
│
├─ 5. LOOP {
│   ├─ 5a. PromptBuilder: 渲染 `RenderedPrompt`
...
│   ├─ 5c. ChatRequestBuilder: 用 `RenderedPrompt + RequestContext + RequestBuildOptions`
│   │     构建正式 `ChatRequest`，并执行 tool / vision capability 预检
...
3. `ChatRequestBuilder` 使用 `RequestBuildOptions { allow_tools: false }`，发送 `tools = []`，强制模型生成纯文本
...
### 6.6 PromptBuilder 与 ChatRequestBuilder

为避免“Prompt 文本渲染”和“Provider 请求组装”在实现期重新耦合，本设计将二者明确拆开。

#### PromptBuilder

`PromptBuilder` 无状态，每轮完整渲染 system prompt 文本段，输出 `RenderedPrompt`。按 `0005 §5.3` 的顺序组装：
...
- `PromptBuilder` 只做渲染，不做存储访问。
- 记忆文本由 `MemoryManager` 产出。
- Tool definitions 不允许被拼进 prompt 文本。

#### ChatRequestBuilder

`ChatRequestBuilder` 负责把 `RenderedPrompt`、`RequestContext`、`ResolvedSessionConfig` 和 request 级策略组装成正式 `ChatRequest`。

```rust
pub struct RequestBuildOptions {
    /// 是否允许本次请求暴露 tools；false 时固定发送 `tools = []`
...
1. 将 `RenderedPrompt` 以前导 `Message::System` 形式插入 `ChatRequest.messages[0]`
2. 将 `ContextManager` 产出的可见消息历史接到其后
3. 按 `RequestBuildOptions.allow_tools` 决定本次 request 是否暴露 tool definitions
4. 对输入内容块做 provider 消息映射：`Text` / `FileContent` 走文本通道，`Image` 走 vision 通道
5. 在 provider 请求前执行 capability 预检：
   - 目标模型不支持 `tool_use` 且本次 request 需要 tools → 直接失败
   - 输入含 `Image` 且目标模型不支持 vision → 直接失败

因此：

- `PromptBuilder` 只负责“系统文本长什么样”
- `ChatRequestBuilder` 只负责“正式请求长什么样”
- 所有模型请求都必须经过 `PromptBuilder + ChatRequestBuilder`
...
### D21: 为什么把 PromptBuilder 和 ChatRequestBuilder 拆开？

如果把 system prompt 渲染、多模态映射、tool 开关、capability 预检都塞进同一个组件，后续实现中很容易重新出现“prompt 文本”和“正式 API request”互相污染的问题。例如：

- tool definitions 被错误拼进 prompt 文本
- `allow_tools=false` 的收尾请求没有统一出口
- vision / tool_use 能力检查散落在 TurnEngine 与 PromptBuilder 之间

拆分后的边界更稳定：

- `PromptBuilder` 只关心系统文本的顺序和格式
- `ChatRequestBuilder` 只关心 provider 请求结构、tools 通道和 capability guard

这样既便于测试，也能保证实时路径、tool loop 收尾路径和 compact 重试路径都经过同一条 request 构建链路。

### D22: 为什么 v1 不采用 Codex-style agent policy？

Codex 中的 agent policy 实际上是多种能力的组合：`approval_policy`、`sandbox_policy`、`exec_policy`、`AGENTS.md` 项目文档发现，以及把这些规则转成 developer instructions 后再暴露给模型。

但本项目 v1 的问题域不同：

- `0005` 明确要求 Agent 核心不访问文件系统，所有输入由调用方提供，因此不应引入 `AGENTS.md` 扫描式策略发现
- `0005` 还明确要求所有已注册 Tool 可被模型自由调用，无额外鉴权机制；这与 Codex 的执行审批语义不是同一个抽象层

因此，v1 保持更小的设计面：

- 项目级约束由调用方通过 `system_instructions` 提供
- 会话级临时约束由 `system_prompt_override` 提供
- 风格约束由 `personality` 提供
- 运行环境事实由 `environment_context` 提供

若未来引入内建 shell/file/network tool 与人工审批流，再单独设计 `ExecutionPolicy` 或 `ToolAccessPolicy`，而不是预先引入一个过宽、语义模糊的 `AgentPolicy`。
...
| `§1.1 无状态核心` / `§4 AgentConfig` | `A6`, `§4.8`, `§6.6`, `§12`, `D22` | Aligned with explicit scope cut | 不扫描 `AGENTS.md`，不引入 Codex-style agent policy；策略类约束由调用方通过 `system_instructions` / `system_prompt_override` / `personality` / `environment_context` 提供 |
...
| `§5.3 System Prompt Builder` | `§5.1`, `§6.6`, `D21` | Aligned with refinement | PromptBuilder 只渲染 system prompt 文本；正式 `ChatRequest` 由 ChatRequestBuilder 统一组装 |
