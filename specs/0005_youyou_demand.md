...
| 2 | Personality | 调用方通过 AgentConfig.personality 传入 | 包裹 `<personality_spec>` 标签（见 Appendix B.3） |
| 3 | Tool Definitions | 已注册 ToolHandler 的元数据 | 序列化为 JSON，通过 API `tools` 参数传递（非 prompt 文本） |
| 4 | Skill List | 已注册且 allow_implicit_invocation=true 的 Skill | 渲染为 Skill Section 文本（见 Appendix B.4） |
| 5 | Active Plugin Info | 已注册的 Plugin 列表 | 渲染为 Plugin Section 文本（见 Appendix B.5） |
| 6 | Memories | MemoryStorage 查询结果 | 注入记忆内容 + 记忆使用指令（见 Appendix B.6） |
| 7 | Environment Context | 调用方通过 AgentConfig.environment_context 传入 | 格式化为 XML（见 Appendix B.7） |
| 8 | Dynamic Sections | TurnStart Hook 的 ContinueWith 返回值 | 直接注入 |
